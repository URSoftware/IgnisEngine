...
- ✅ **F3.5 — Casca do editor: ciclo de vida de projeto + mecânicas (2026-06-15):** paridade da casca `IgnisEditorApp` com o editor Swing, tudo **aditivo** (sem tocar `com.ignis.core`).
  - **Tela de seleção de projeto** (`FxProjectStartupDialog`) ao abrir o editor — análoga ao `showStartupDialog` do Swing, porém com **lista visual** de projetos + recentes (★). Mostrada via `Platform.runLater` após `stage.show()`, em laço até carregar/sair.
  - **Persistência estilo VSCode** (`EditorPrefs`, em `~/.ignis/editor-prefs.json` via `org.json`): último projeto + recentes (MRU, dedupe, truncado em 10, `clearMissing`). Submenu **Abrir recente** dinâmico.
  - **Ciclo de vida** no menu Arquivo: Novo (`createNew`+estrutura+Square central), Abrir (FileChooser), Selecionar (dialog), **Salvar** (Ctrl+S — antes o FX **nunca** gravava o `.ignis`), **Salvar como** (Ctrl+Shift+S, ajusta `projectName`), **Fechar** (libera `ScriptManager`, limpa estado e volta à seleção) e **Trocar**. `currentProject` agora é mantido; `syncEntitiesToScene()` espelha `doSaveProject` com `scene.clear()`+`addEntity`.
  - **Mecânicas de edição** (menu **Cena** + **ContextMenu** na Hierarchy + teclas com foco na árvore): criar via `EntityFactory` (Square/Circle/Triangle/Star/Pentagon/Player/Camera), **duplicar** (Ctrl+D, `saveProperties`/`loadProperties`), **deletar** (Delete), **renomear** (F2), **reordenar z-order** (`Game.moveEntity*`). `Camera` criada é registrada via `game.addCamera`; cameras residuais são limpas ao trocar/fechar projeto.
  - **Asset Browser**: `TreeView<File>` da pasta do projeto (recursão com guarda de symlink/profundidade/ciclo), abre arquivos no duplo-clique. Painel esquerdo agora é um `SplitPane` vertical (Hierarchy + Assets).
  - Build `mvnw compile` = **SUCCESS** (86 fontes). Revisão adversarial aplicada (re-seleção pós-mover, cameras órfãs, save-as identidade, guarda do asset tree).
  - **Validado antes da F4 (checklist):** seleção visual de projeto ✅, persistência último/recentes ✅, novo/abrir/salvar/salvar-como/fechar/trocar ✅, criar/duplicar/deletar/renomear/reordenar ✅, ContextMenu ✅, Asset Browser ✅, janelas-ferramenta seguem abrindo ✅, Play/Stop + input ✅ (validar em GUI). **Pendente para paridade total (pós-F4):** prefabs (`PrefabManager`), drag-and-drop na Hierarchy, MarkdownViewer FX, import-imagem→sprite/merge/anexar-script-a-objeto, assistente de IA (`AuxiliaryPanel`).
- **F4 — Tema e limpeza:** CSS escuro (substitui cores hardcoded), layout persistido (SplitPane/Stage), remover `javafx-swing`. `runtime/GameRuntime` pode permanecer AWT (menor footprint) — decidir.
  ...
  - **Dívida técnica registrada:** `IgnisProjectIO.save`/`.replace(".ignis","")` (core) relocaliza para `projects/<nome>/` e usa replace literal — corrigir no core (fora do escopo aditivo da casca); extrair `imageeditor/ToolType` para aposentar o `PaintCanvas` Swing.
